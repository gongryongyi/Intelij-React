...
import static com.ohgiraffers.comprehensive.common.exception.type.ExceptionCode.ExceptionCode.*;
...
public class ReviewService {
    ...
    public Page<ReviewResponse> getReviews(final int page, final Long productCode) {

        final Page<Review> reviews = reviewRepository.findByProductProductCode(getPageable(page), productCode);

        return reviews.map(review -> ReviewResponse.from(review));
    }
    ...
    @Transactional(readOnly = true)  //순수히 조회만
